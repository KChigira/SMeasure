...
public class SMeasureController {
	...
	@FXML
	private TableColumn<TableValue, String> col1, col2, col3, col4, col5;
...
//////////////////////////////////////////////
	...
	protected void init() {
		...
		imageview.setFitWidth(anchor.getWidth() - 530);
        ...
        col5.setCellValueFactory(new PropertyValueFactory<TableValue, String>("sm"));

	}

	private void processImage() {
		this.underLabel.setText("Calculating...");
		...
		for(int i = 0; i<contours.size(); i++) {
			if(Geometry.contourArea(contours.get(i)) > min_cnt){
        ...
        for(int i=0; i<l_contours.size(); i++) {
        	...
        	RotatedRect rr = Geometry.minAreaRect(mop2);
        ...
        for(int i=0; i<outer_l_contours.size(); i++) {
	        ...
	        for(int j = 0; j<2; j++) {
		        ...
		        for(int k=0; k<2; k++) {
		        	...
		        	RotatedRect l_rr = Geometry.minAreaRect(mop22);
	        ...
	        if(od[0] != -1.0) {
	        	...
	        	Imgproc.putText(masureImage, String.valueOf(values.size() - 1), corner[0], Imgproc.FONT_HERSHEY_SIMPLEX, 2.0, new Scalar(150, 150, 255), 2);
	        }


        }

        this.underLabel.setText(this.values.size() + " region was detected.");
		...
		if(adjust.isSelected()) {
			...
			for(int i=0; i<this.values.size(); i++) {
				if(this.values.get(i).getRectangle().contains(ap)) {
					...
					this.showImage();
					this.underLabel.setText("Result of No." + i);
					break;
				}
			}
		}
	...
	public void keyPress(KeyEvent ke) {
		...
		case Z:
			...
			this.underLabel.setText("A blank value was recorded.");
			break;
		default:
			break;
	...
	public void writeValue() {
		...
		this.currentSelectedRow++;
		this.underLabel.setText("A measured value was recorded.");
	}
	@FXML
	public void eraseValue() {
		...
		this.currentSelectedRow--;
		this.underLabel.setText("A record was deleted.");
	}
	@FXML
	public void saveValue() {
	    ...
	    this.underLabel.setText("Data was saved.");
	}
	...
	public void savePicture() {
	    ...
	    if(Imgcodecs.imwrite(f.getAbsolutePath(), this.processedImage)) {
	    	this.underLabel.setText("Pictures were saved.");
	    }else {
	    	this.underLabel.setText("Sorry, saving pictures was failed.");
	    }
	}
	...
	public void savePictureWithMarker() {
		...
		while(this.picturefile == null) {
			...
			dc.setTitle("Select a location to save the image.");
	...
	private void setValue(Values val) {
		this.os.setText(String.format("Outer diameter of minor axis (a1): %1$.2f mm", val.getShorterOuterDiameter() * mmperpx));
		this.ol.setText(String.format("Outer diameter of major axis (b1): %1$.2f mm", val.getLongerOuterDiameter() * mmperpx));
		this.is.setText(String.format("Inner diameter of minor axis (a2): %1$.2f mm", val.getShorterinnerDiameter() * mmperpx));
		this.il.setText(String.format("Inner diameter of major axis (b2): %1$.2f mm", val.getLongerInnerDiameter() * mmperpx));
		this.sm.setText(String.format("Section modulus (SM): %1$.2f mm3", val.getSectionModulus() * Math.pow(mmperpx, 3)));
	}
